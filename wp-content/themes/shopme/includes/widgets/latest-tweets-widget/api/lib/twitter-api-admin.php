...
function twitter_api_admin_render_header( $subheader, $css = '' ){
    ?>
     <div class="wrap">
        <h2><?php echo esc_html__('Twitter API Authentication Settings','shopme')?></h2>
...
function twitter_api_admin_render_form(){
    ...
    <form action="<?php echo twitter_api_admin_base_uri()?>" method="post">
        <p>
            <label for="twitter-api--consumer-key"><?php echo __('OAuth Consumer Key','shopme');?>:</label><br />
            <input type="text" size="64" name="saf_twitter[consumer_key]" id="twitter-api--consumer-key" value="<?php echo esc_html($consumer_key)?>" />
        </p>
        <p>
            <label for="twitter-api--consumer-secret"><?php echo __('OAuth Consumer Secret','shopme');?>:</label><br />
        ...
        <p>
            <label for="twitter-api--access-key"><?php echo __('OAuth Access Token','shopme');?>:</label><br />
            <input type="text" size="64" name="saf_twitter[access_key]" id="twitter-api--access-key" value="<?php echo esc_html($access_key)?>" />
        </p>
        <p>
            <label for="twitter-api--access-secret"><?php echo __('OAuth Access Secret','shopme');?>:</label><br />
        ...
        <p class="submit">
            <input type="submit" class="button-primary" value="<?php echo esc_html__('Save settings','shopme')?>" />
        </p>
        <small>
            <?php echo esc_html__('These details are available in','shopme')?> 
            <a href="https://dev.twitter.com/apps"><?php echo esc_html__('your Twitter dashboard','shopme')?></a>
        </small>
    </form>
    <?php
}
...
function twitter_api_admin_render_login( $consumer_key, $consumer_secret ){
    ...
    echo '<p><a class="button-primary" href="',esc_html($href),'">'.esc_html__('Connect to Twitter','shopme').'</a></p>';
    echo '<p>&nbsp;</p>';
}
...
function twitter_api_admin_render_page(){
    if ( ! current_user_can('manage_options') ){
        twitter_api_admin_render_header( __("You don't have permission to manage Twitter API settings",'shopme'),'error');
    ...
    try {
        ...
        if( ! $consumer_key || ! $consumer_secret ){
            throw new Exception( __('Twitter application not fully configured','shopme') );
        }
        ...
        if( ! $access_key || ! $access_secret ){
            twitter_api_admin_render_header( __('Plugin not yet authenticated with Twitter','shopme'), 'error' );
        ...
        // else we have auth - verify that tokens are all still valid
            ...
            twitter_api_admin_render_header( sprintf( __('Authenticated as @%s','shopme'), $me['screen_name'] ), 'updated' );
        }
