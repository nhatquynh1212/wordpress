 ...
 */
class MAD_Datetime_Field extends MAD_Text_Field {
	...
	public static function admin_register_scripts() {
		...
		wp_register_style( 'wp-datepicker', MAD_CSS_URL . 'datepicker.css', array( 'jquery-ui-core', 'jquery-ui-theme' ), '1.8.17' );
		...
		wp_register_script( 'mad-datetime', MAD_JS_URL . 'datetime.js', array( 'jquery-ui-datepicker-i18n', 'jquery-ui-timepicker-i18n' ), MAD_VER, true );
		wp_register_script( 'mad-date', MAD_JS_URL . 'date.js', array( 'jquery-ui-datepicker-i18n', 'jquery-ui-timepicker-i18n' ), MAD_VER, true );
		wp_register_script( 'mad-time', MAD_JS_URL . 'time.js', array( 'jquery-ui-timepicker-i18n' ), MAD_VER, true );
		...
		foreach ( $handles as $handle ) {
			self::localize_script( "mad-$handle", 'MAD_' . ucfirst( $handle ), $data );
		}
	}
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_script( 'mad-datetime' );
	}
	...
	public static function html( $meta, $field ) {
		...
		if ( $field['timestamp'] ) {
			...
			$output .= sprintf(
				'<input type="hidden" name="%s" class="mad-datetime-timestamp" value="%s">',
		...
		if ( $field['inline'] ) {
			$output .= '<div class="mad-datetime-inline"></div>';
		}
