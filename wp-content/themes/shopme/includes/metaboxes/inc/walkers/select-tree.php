 ...
 *
 * @uses MAD_Walker_Select
 */
class MAD_Walker_Select_Tree {
	...
	function display_level( $options, $parent_id = 0, $active = false ) {
		...
		$field      = $this->field;
		$walker     = new MAD_Walker_Select( $this->db_fields, $field, $this->meta );
		$attributes = MAD_Field::call( 'get_attributes', $field, $this->meta );

		$children = $options[ $parent_id ];
		$output   = sprintf(
			'<div class="mad-select-tree %s" data-parent-id="%s"><select %s>',
			$active ? '' : 'hidden',
			$parent_id,
