...
jQuery( function ( $ ) {
	...
	MediaStatus = views.MediaStatus = Backbone.View.extend( {
		tagName: 'span',
		className: 'mad-media-status',
		template: wp.template( 'mad-media-status' ),
	...
	MediaButton = views.MediaButton = Backbone.View.extend( {
		className: 'mad-add-media button',
		tagName: 'a',
		events: {
			click: function () {
				...
				this._frame = wp.media( {
					className: 'media-frame mad-media-frame',
					multiple: true,
	...
	MediaItem = views.MediaItem = Backbone.View.extend( {
		tagName: 'li',
		className: 'mad-media-item',
		template: wp.template( 'mad-media-item' ),
		initialize: function ( options ) {
			this.controller = options.controller;
			this.render();
		...
		events: {
			...
			'click .mad-remove-media': function ( e ) {
				this.controller.removeItem( this.model );
				return false;
			},

			'click .mad-edit-media': function ( e ) {
				// Destroy the previous collection frame.
				if ( this._frame ) {
					//this.stopListening( this._frame );
					this._frame.dispose();
	...
	function initMediaField() {
		new MediaField( {input: this, el: $( this ).siblings( 'div.mad-media-view' )} );
	}


	$( ':input.mad-file_advanced' ).each( initMediaField );
	$( '.mad-input' )
		.on( 'clone', ':input.mad-file_advanced', initMediaField );
} );
