...
jQuery( function ( $ ) {
	...
	var cloneIndex = {
		/**
		 * Set index for fields in a .mad-clone
		 * @param $clone .mad-clone element
		 * @param index Index value
		 */
		set: function ( $clone, index ) {
			$clone.find( ':input[class|="mad"]' ).each( function () {
				var $field = $( this );

				// Name attribute
				var name = $field.attr( 'name' );
				if ( name && ! $field.closest( '.mad-group-clone' ).length ) {
			...
			var $address = $clone.find( '.mad-map-goto-address-button' );
			if ( $address.length ) {
		 ...
		 * @param $container .mad-input container
		 * @return integer
		 */
	...
	function clone( $container ) {
		var $last = $container.children( '.mad-clone:last' ),
			$clone = $last.clone(),
			$input = $clone.find( ':input[class|="mad"]' ),
		...
		$input.each( function () {
			...
			} else if ( ! $field.hasClass( 'mad-hidden' ) ) {
				// Reset value
				$field.val( '' );
			}
		} );
	...
	function toggleRemoveButtons( $container ) {
		var $clones = $container.children( '.mad-clone' );
		$clones.children( '.remove-clone' ).toggle( $clones.length > 1 );

		// Recursive for nested groups.
		$container.find( '.mad-input' ).each( function () {
	...
	function toggleAddButton( $container ) {
		var $button = $container.find( '.add-clone' ),
			...
			numClone = $container.find( '.mad-clone' ).length;
	...
	$( '#wpbody-content' )
		...
		.on( 'click', '.add-clone', function ( e ) {
			...
			var $container = $( this ).closest( '.mad-input' );
			clone( $container );
		...
		.on( 'click', '.remove-clone', function ( e ) {
			...
			var $this = $( this ),
				$container = $this.closest( '.mad-input' );

			// Remove clone only if there are 2 or more of them
			if ( $container.children( '.mad-clone' ).length < 2 ) {
				return;
			}
	...
	$( '.mad-input' ).each( function () {
		var $container = $( this );
		toggleRemoveButtons( $container );
		toggleAddButton( $container );

		$container
			.data( 'next-index', $container.children( '.mad-clone' ).length )
			.sortable( {
				handle: '.mad-clone-icon',
				placeholder: ' mad-clone mad-clone-placeholder',
				items: '.mad-clone',
