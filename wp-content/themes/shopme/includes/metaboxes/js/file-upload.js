...
jQuery( function ( $ ) {
	...
	UploadButton = views.UploadButton = Backbone.View.extend( {
		className: 'mad-upload-area',
		tagName: 'div',
		template: wp.template( 'mad-upload-area' ),
		render: function () {
		...
		initialize: function ( options ) {
			...
			this.el.id = _.uniqueId( 'mad-upload-area-' );
			this.render();

			//Areas
			this.dropzone = this.el;
			this.browser = this.$( '.mad-browse-button' )[0];
	 ...
	 */
	function init() {
		new FileUploadField( {input: this, el: $( this ).siblings( 'div.mad-media-view' )} );
	}

	$( ':input.mad-file_upload' ).each( init );
	$( '.mad-input' )
		.on( 'clone', ':input.mad-file_upload', init )
} );
