...
jQuery( function ( $ ) {
	...
	function getOriginalId( $el ) {
		var $clones = $el.closest( '.mad-clone' ).siblings( '.mad-clone' ),
			id = '';
		$clones.each( function () {
			var currentId = $( this ).find( '.mad-wysiwyg' ).attr( 'id' );
	...
	$( ':input.mad-wysiwyg' ).each( update );
	$( '.mad-input' ).on( 'clone', ':input.mad-wysiwyg', update );
} );
