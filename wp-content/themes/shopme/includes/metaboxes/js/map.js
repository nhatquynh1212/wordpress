(function ( $ ) {
	...
	MapField.prototype = {
		...
		initDomElements: function () {
			this.canvas = this.$container.find( '.mad-map-canvas' )[0];
			this.$coordinate = this.$container.find( '.mad-map-coordinate' );
			this.$findButton = this.$container.find( '.mad-map-goto-address-button' );
		...
		addListeners: function () {
			 ...
			 *      ('resize' Event)
			 */
			$( window ).on( 'mad_map_refresh', function () {
				that.refresh();
			} );
